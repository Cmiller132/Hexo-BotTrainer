#!/usr/bin/env bash
# Pair-ceiling sweep: fewer/fatter serve groups vs dispatch overhead.
# Python pack (rust_pack has its own in-Rust grouping ceiling; port if this wins).
set -eu
...
for C in 1.5e8 3.0e8 6.0e8; do
  echo "=== vbs=48, ceiling=$C ==="
  HEXFIELD_PAIR_CEILING=$C "$PY" scripts/_hexfield_selfplay_throughput.py "$CKPT" 512 192 60 1024 192 48
done
