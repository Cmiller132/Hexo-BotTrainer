#!/usr/bin/env bash
# WSL-side supervisor for the dense_cnn target_64x4 FAST run. Identical guardrails
# to supervise_target_96x6_wsl.sh (resume-from-latest, circuit breaker, completion
...
CONFIG="${CONFIG:-$ROOT/configs/dense_cnn_model1_target_64x4.toml}"
RUNDIR="${RUNDIR:-$ROOT/runs/dense_cnn_model1_target_64x4}"
...
log "WSL SUPERVISOR start (pid=$$) run=dense_cnn_model1_target_64x4 config=$CONFIG epochs=$LOOP_EPOCHS"
