#!/usr/bin/env bash
# Generate the SealBot-bootstrapped prefit for the 64x4+P7 architecture (its shape
...
cd "$ROOT"
mkdir -p runs/dense_cnn_model1_target_64x4/checkpoints
python scripts/bootstrap_dense_cnn_sealbot.py \
  --config configs/dense_cnn_model1_target_64x4.toml \
  --out runs/dense_cnn_model1_target_64x4/checkpoints/bootstrap_sealbot_prefit.pt \
  --sealbot-path "$SEALBOT" \
  --target-samples 16000 --threads 32 --turn-time-ms 100 --prefit-epochs 8 2>&1 | tail -40
echo "=== bootstrap rc=${PIPESTATUS[0]} ==="
ls -la runs/dense_cnn_model1_target_64x4/checkpoints/bootstrap_sealbot_prefit.pt 2>/dev/null
