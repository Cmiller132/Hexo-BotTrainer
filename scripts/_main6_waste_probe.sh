#!/usr/bin/env bash
# Waste-fraction sweep: fewer/fatter serve groups (padding absorbed by idle GPU).
set -eu
...
export HEXFIELD_PAIR_CEILING=3.0e8
...
for W in 0.35 0.6 1.0; do
  echo "=== vbs=48, waste=$W ==="
  HEXFIELD_WASTE_FRACTION=$W "$PY" scripts/_hexfield_selfplay_throughput.py "$CKPT" 512 192 60 1024 192 48
done
