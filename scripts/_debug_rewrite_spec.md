...
- The deployed dashboard runs in WSL behind a port proxy on :8080 with a single CPU-only
...
- Everything pinned by `tests/test_debug_infer.py` keeps passing unchanged:
  ...
  `analyze_position` response keys (incl. `value_swapped`, `optimism`, stvalue keys
  {'4','12','24'} for hexgt), `search_position` contract (noise-free, bit-deterministic
...
- JS verification on Windows is `node --check app.js` only; Python via
  `python -m pytest tests/test_debug_infer.py tests/test_dense_cnn_debug_artifacts.py -q`
  (frontend web.py import skips on Windows — that is expected; full run is WSL
  `scripts/_run_all_tests.sh`).
...
- B1. `app.js` (~3905-3908) decodes moves_left with hardcoded `*80`; the real cap is
  `MOVES_LEFT_CAP = 512` (`packages/dense_cnn_restnet/.../constants.py:27`). Decode as
...
- B4. The "optimism probe" UI block (owner-swap Σ with ok/warn coloring) is a misreading:
  the owner-swap P0/P1 probe is off-distribution-confounded (FirstStone parity) and must
  NOT be presented as optimism. The fields stay in the API (tests pin them) but the UI
  relabels them "owner-swap consistency probe (off-distribution; not an optimism
  metric)" with no ok/warn coloring.
...
- Per-root-child Q for the production `search` op is NOT extractable (the Rust MCTS
  exposes only root aggregates — `dense_cnn_restnet/.../mcts.py`); the existing `w` field
...
- INPUTS tab is dense-lineage only (hexgt is graph-featurized, no 13×41×41 planes);
  planes are returned only when explicitly requested (`planes:true` on analyze).
- "Recorded" trajectory series stays hexgt-only (only hexgt runs write
  eval/epoch_*_examples.json); for dense runs the Game Error Sweep's reeval series is the
  replacement.
...
- Mode bar `#dbgModeBar`: segmented control of EXCLUSIVE base heat modes (radio
  ...
  prior−target diverging), `childq` (7, root-child Q from last search_tree), `plane`
  (8, input plane; shows a plane <select> `#dbgPlaneSelect` when active, dense only),
  plus `none` (0). Additive toggles (checkboxes, independent of base mode):
...
- HEADS `#dbgTabHeads`: value scalar + chip `#dbgValueChip`; 65-bin dist chart
  ...
  probe block relabeled per B4 (hexgt only, hidden for dense); STV rows `#dbgStvRows`
...
- INPUTS `#dbgTabInputs`: 13 mini-heatmaps (one per featurizer plane, named per
  constants.py order) rendered client-side from analyze `input_planes`; dense lineage
  only, panel shows "n/a for hexgt lineage" otherwise; loads lazily (first activation
  triggers one analyze with `planes:true`).
...
- Trajectory `#dbgDockChart`: hand-rolled SVG, shared x-axis = ply. Series: re-eval
  value_p0 (accent), recorded root_value_p0 (yellow, hexgt only), policy KL per ply
...
1..8             heat base modes         0               mode none + clear toggles
Shift+1..8       solo (mode + clear      L               log color scale
                 additive toggles)       U               undo last injected move
G                return to recorded game P               pin current position
C                focus COMPARE tab       I               focus INPUTS tab
K or Ctrl+K      command palette         ?               shortcut overlay #dbgHelp
...
S9. INPUTS tab (dense only, lazy `planes:true` analyze).
...
which is plain json readable on the Windows side too — sidecar read happens in web.py,
...
### 4.1 debug_infer.py
- Add `MOVES_LEFT_CAP` plumb-through: `_model_meta` (or the dense meta builder) gains
  `moves_left_cap` (import from dense constants when lineage is dense; null for hexgt)
  and `param_count`. Do NOT change any existing meta key.
- Add `DebugSearchNode` + `search_tree_position(loaded, action_ids, *, visits, c_puct,
  ...
  * Reuses the exact per-position featurize+forward wrappers analyze/search already
    build for both lineages (one net call per expansion; batch of 1 is fine on CPU at
    ≤20k visits — document that 4096 visits ≈ the practical interactive ceiling).
  * Deterministic: seeded `random.Random(seed)` used ONLY for tie-breaking among
    exactly-equal PUCT scores; no Dirichlet noise; FPU/temperature neutral, matching
    the existing debug search settings as closely as practical.
...
- DO NOT touch `load_checkpoint`, `analyze_position` (except the additive
  `planes:bool=False` kwarg returning `input_planes` for dense), or
...
- Pinboard (S2), journal (S3), palette (S1), checkpoint sweep dock tab (S4),
  prefetch (S7), record/file hotkeys (S8), INPUTS tab (S9), overlay ergonomics (S6).
...
- No frameworks, no bundler, no build step, no new npm/pip dependencies. Stdlib +
  the already-vendored WSL venv (torch/numpy/compact_io) only.
- Do not start, restart, or probe the deployed :8080 dashboard or the WSL worker.
  Verification = `node --check`, pytest (Windows subset), WSL `scripts/_run_all_tests.sh`
  run by the owner.
- Do not modify the Rust engine/rust_bridge, the production MCTS, selfplay/training
  code, or anything under packages/dense_cnn_restnet, packages/hexo_models,
  packages/hexo_train, packages/hexo_utils.
