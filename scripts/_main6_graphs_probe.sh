#!/usr/bin/env bash
# CUDA-graph serve: parity vs graph-off baseline + throughput.
set -eu
...
echo "--- parity: + cuda_graphs vs copy-stream baseline (1e-6) ---"
HEXFIELD_CUDA_GRAPHS=1 "$PY" scripts/_hexfield_serve_ref.py check /tmp/main6_cs_ref.npz 1e-6

echo "=== throughput: vbs=48 + cuda_graphs ==="
