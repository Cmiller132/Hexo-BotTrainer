...
$outdir = Join-Path $root 'corpus9_shards'
New-Item -ItemType Directory -Force -Path $outdir | Out-Null

# DEEP + NARROW profile (the config that harvests forced wins efficiently):
#  scope  = first 9 plies of the corpus, D6-position-deduped
#  width  = vcf_pair_complete (narrow, deep)
#  horizon= UNBOUNDED (semantic_horizon = u32::MAX) -- the depth that wins
#  cap    = 8,000,000 nodes/pos (headroom for 9-ply forcing lines)
#  win_line PV is emitted for every certified win.
...
$start = Get-Date
Write-Output "launched 16 deep-9ply workers at $start -> $outdir"
...
Write-Output "all deep-9ply workers exited; elapsed_s=$([math]::Round($elapsed,0))"
