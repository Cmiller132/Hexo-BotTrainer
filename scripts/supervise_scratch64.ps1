<#
.SYNOPSIS
  Host-side autonomy supervisor for the dense_cnn Model 1 (scratch_64) run.
...
param(
    ...
    [string]$ConfigPath = "configs\dense_cnn_model1_scratch_64.toml",
    ...
    [int]$MaxNoProgressRelaunches = 3,       # N relaunches with no new epoch checkpoint -> halt (slow-loop guard)
...
$runName = Get-ConfigString -Section "run" -Key "name" -Default "dense_cnn_model1_scratch_64"
...
function Get-Trainer {
    Get-CimInstance Win32_Process | Where-Object {
        ...
        $_.CommandLine -match "scratch_64"
