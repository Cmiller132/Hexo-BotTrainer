#!/usr/bin/env bash
# WSL-side supervisor for the dense_cnn target_96x6 run — equivalent guardrails to
# the Windows scripts/supervise_target_96x6.ps1, but launches the trainer under the
# WSL venv so TensorRT FP16 actually engages (native Windows has no py3.14 TRT
# wheel). The PS supervisor can't drive a WSL trainer (WSL procs are invisible to
# Win32_Process), hence this.
#
# Guardrails (match the PS supervisor):
#  - resume-from-latest: inject [checkpoint].resume_from=<newest epoch_*.pt> each
#    relaunch; first launch (no epoch ckpt) uses the config's initialize_from (the
#    SealBot prefit) -> bootstrapped, not random.
#  - circuit breaker: 3 consecutive fast crashes (<180s), OR >6 crashes/hour, OR
#    MAX_NO_PROGRESS=5 relaunches with no new epoch checkpoint -> write halt flag.
#  - completion guard: latest_epoch+1 > loop.epochs -> completed flag, stop.
#  - crash artifacts: per-crash err/out tail saved under diagnostics/crash_artifacts.
#  - RAM floor watchdog: background sampler logs free RAM; WSL2 28GB cap + Linux OOM
#    protect the host (the Windows private-RAM watchdog can't see WSL procs).
#  - single instance via a lockfile.
set -uo pipefail

ROOT="${ROOT:-/mnt/e/Hexo-BotTrainer}"
VENV="${VENV:-/root/.venvs/hexo-bottrainer-wsl}"
CONFIG="${CONFIG:-$ROOT/configs/dense_cnn_model1_target_96x6.toml}"
RUNDIR="${RUNDIR:-$ROOT/runs/dense_cnn_model1_target_96x6}"
...
# SealBot worker (sys.executable) must import game + minimax_cpp from the checkout.
...
set_resume(){ # inject [checkpoint].resume_from in-place (matches PS Set-ResumeFrom)
  "$PY" - "$CONFIG" "$1" <<'PY'
...
log "WSL SUPERVISOR start (pid=$$) run=dense_cnn_model1_target_96x6 config=$CONFIG epochs=$LOOP_EPOCHS"
...
# RAM watchdog (background): sample free RAM; log; WSL OOM is the hard backstop.
( while :; do
