...
(`E:\Hexo-BotTrainer\runs\dense_cnn_restnet_main_3`) on 2026-06-11. Where this file
and the design draft disagree, THIS FILE wins.
...
- `row.selfplay.buffer` — the UNIFORM per-head loss carrier for ALL lineages
  (`loss_total, loss_policy, loss_value, loss_opp, loss_stvalue_<h>`; hexgt adds
  `samples, cap, window_epochs, window_span, decay, train_steps, train_batch,
  train_samples_per_epoch, optimism_sum_mean`). dense_cnn buffers are loss-only —
...
`diagnostics/dense_cnn.selfplay.epoch_{N:06d}.json` (~120KB; only on dense-lineage
runs) carries useful stats NOT in `_selfplay_epoch_summary`:
...
extras (`opponent_elapsed_seconds`, `rounds`, `dense_forward_batches`,
`dense_decisions`) were inventoried and REJECTED: marginal value, would not earn
screen space.
...
```
- `selfplay_extras`: read `diagnostics/dense_cnn.selfplay.epoch_{epoch:06d}.json`
  via `_read_json_file`; None when absent/non-dict. Curate EXACTLY:
...
- `manifest`: read `<run_dir>/manifest.json` (`utf-8-sig`, json, dict-guarded; None
  ...
  keys simply omitted (hexgt/hexgnn manifests differ; never KeyError).
- `checkpoint`: name = the history row's `checkpoint.name` or
  ...
  only — covers the HEXO_DEBUG_RUN_ROOT worktree/mirror split, §0.2) and stat that.
...
1. Full epoch: write `diagnostics/epoch_000002.json` (with
   ...
   `diagnostics/dense_cnn.selfplay.epoch_000002.json` (including a
   `temperature_control` dict AND an `npz_writes` list + `mcts_diagnostics` dict),
   `diagnostics/dense_cnn.evaluation.epoch_000002.json`, a previous
   `diagnostics/epoch_000001.json`, `checkpoints/epoch_000002.pt` (a few bytes),
   and a `manifest.json` with a `model.config` subset. Assert: response shape;
...
Run the suite the project way; tests are authoritative in the WSL venv (per repo
memory) — at minimum ensure the new file passes locally and existing
`test_frontend_training_artifacts.py` still passes.
...
4. BUFFER — only when `buf.samples` or `buf.cap` is finite (hexgt pool; dense
   loss-only buffers hide this group — draft requirement): pool `samples/cap`,
...
8. DIAGNOSTICS (lazy, from `/api/training/epoch` `selfplay_extras` + `diagnostics`):
   ...
   a single muted line; when the payload has `selfplay_extras: null` (hexgt runs,
   running epoch) the group is hidden.
9. MODEL — two layers:
...
B4. `/api/training/epoch` returns the P3.1 contract shape for a real epoch
    ...
    pre-existing tests still pass (authoritative in the WSL venv).
...
- hexgt runs have no `dense_cnn.selfplay.epoch_*.json` → `selfplay_extras` null →
  DIAGNOSTICS group hidden; dense runs have loss-only buffers → BUFFER/CALIBRATION
