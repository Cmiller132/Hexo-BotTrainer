...
`dashboard.py` on 2026-06-11; if code and spec disagree on a FIELD NAME, the names
here win (they were read from the live code).
...
- Do NOT git commit. Do NOT touch the live dashboard on :8080 (it runs old code in
  WSL; leave it alone). Smoke testing uses a private port (§9).
- Tests are authoritative ONLY in the WSL venv:
  `wsl -- bash -lc 'cd /mnt/e/Hexo-BotTrainer-hexgt && /root/.venvs/hexgt-build/bin/python -m pytest tests/test_hexo_runner_match_mode.py -x -q' > out.txt`
  (single-quote, redirect to a file — wsl.exe truncates long stdout).
...
- `search` (debug_infer.search_position → `_search_dense`/`_search_hexgt`): fresh
  reproducible CPU MCTS, NO root noise, temperature 1.0. Returns
...
- `analyze` (debug_infer.analyze_position): one forward. Returns (dense)
  ...
  LEGAL cells only (rows are built from legal_action_ids). hexgt lineage returns the
  same `policy` row shape.
...
- Run via the WSL venv command in the header; also run
...
A2. Checkpoint vs Checkpoint: pick two different checkpoints of
    `dense_cnn_restnet_main_3` (e.g. e1 vs e2) at low visits; Start; the board
...
A7. History and Debug screens are pixel-identical; `node --check` passes;
    `py_compile` passes; the WSL pytest commands in §3.6 pass; no console
...
In WSL (`wsl -- bash -lc '…'`, output to files, short commands — wsl.exe
truncates): read `scripts/_dashboard_launch.sh` for the venv/env pattern, then
start a PRIVATE server: cwd `/mnt/e/Hexo-BotTrainer`, venv
`/root/.venvs/hexgt-build/bin/python -m hexo_frontend.web --port 8901`
(setsid + log file per the launch script; NEVER touch :8080). Then with curl:
1. `GET /api/state` → 200, `version` present.
2. `POST /api/new` ckpt-vs-ckpt: `dense_cnn_restnet_main_3` `epoch_000001.pt`
...
Kill the server (pkill -f "port 8901" CAREFULLY — match the exact cmdline per
the WSL memory note), report every check with its observed value.
