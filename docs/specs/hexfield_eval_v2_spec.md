# hexfield — multi-stage strength evaluation (eval-v2) specification

Status: **PROPOSED — design spec for a statistically-sound, multi-stage strength evaluation of the
hexfield lineage.** Authored 2026-06-13. Supersedes the current per-epoch 16-game vs-prior arena
(`packages/hexfield/python/hexfield/evaluation.py`) for the purpose of *measuring* strength. An
adversarial statistician reviewed the naive first-cut design; what follows is the **corrected**
design (converged pairing-aware Bradley-Terry, family-wise error control via a single primary
hypothesis, honest significance accounting). The naive version is recorded only where needed to
...
> changes the run. The live run is `hexfield_main_1` and this evaluation never touches it.
> See §8 (the load-bearing "purely-eval" contract).

Working name: **eval-v2** (the strength-measurement harness for hexfield).
...
2. **16 games cannot resolve the moves that matter.** Both real production evals straddle 0.5 —
   ep5 vs ep4 = 9-7 (0.5625), ep10 vs ep9 = 6-10 (0.375) (hexfield-goal memory; the run was in
...
- **BC prefit**: `runs/hexfield_bc_1/checkpoint_epoch2.pt` — the warm-start the RL run began from
  (hexfield-goal memory: production warm-starts `checkpoint_epoch2.pt`). "Has RL improved on the
  starting point at all?" is answered against this anchor.
- **ep5**: the epoch-5 checkpoint of the live run (`runs/hexfield_main_1/checkpoints/epoch_000005.pt`).
  An early-but-post-warm-start fixed reference. (The exact early anchor epoch is a §9 decision
  point; ep5 is the default because it is the first epoch the current arena evaluated.)
...
- **Wilson CI** on the pair-level rate (the `wilson_ci` helper, ported from
  `scripts/_wf_h2h2_arena.py:93-100`), reported per opponent.
...
The BT fit (§4.4) is the corrected, converged, pairing-aware fit — **not** the legacy
non-converging GD of `scripts/_wf_r4_bt_rating.py`. It yields:
...
### 4.1 Bradley-Terry must CONVERGE (fix the legacy GD)

The legacy `scripts/_wf_r4_bt_rating.py` fits BT by **fixed-step gradient descent** (500 iters,
step 0.002, lines 63-67). This **does not converge** — at the optimum `max|grad|` is ~0.30, nowhere
near zero, so the ratings are biased and the Fisher-information SEs computed from them (lines 69-77)
are meaningless. **Do not inherit this.**
...
eval-v2 is a **standalone runner under `scripts/`** (e.g. `scripts/_hexfield_eval_v2.py`). It is
**NOT auto-run inside the training pipeline** — the pipeline's per-epoch `evaluate_epoch`
(evaluation.py:69) is untouched, so launching eval-v2 never blocks, delays, or interrupts a
...
- **The multi-root API already exists.** `HexfieldMctsSession.search` takes
  `game_keys: Vec<u64>` + a list of `states` and searches them all in one call with cross-game
  leaf batching (`packages/hexfield/rust/src/search.rs:512-544`; it already validates
...
- **hexfield-vs-checkpoint reuses `_play_pair`'s machinery.** `_play_pair` (evaluation.py:23)
...
- **hexfield-vs-SealBot is a near-mechanical port of the dense concurrent loop.** The blueprint is
  `packages/dense_cnn_restnet/python/dense_cnn_restnet/evaluation.py:327-391`: each round, the
  positions where the **model** is to move are searched in one batched
  `mcts_session.run([...keys], [...states])` call (here `HexfieldMctsSession.search`), while
  **SealBot's** moves are drained **serially per game** via the SealBot adapter
  (`SealBotPlayer.decide`, sealbot.py:116; one isolated worker per game, exactly as the dense loop
  does). SealBot's fixed 50 ms minimax is independent per game and is not the bottleneck (the
  batched model forward is); §1.1's load-caveat is the price of that serial drain.
...
disjoint across opponents and epochs (a seed-stride scheme like the dense reference ladder's,
evaluation.py:46-70 in the dense module).

---

## 7. Implementation file map

All eval-v2 code is **eval-only** and lives in disjoint files (no edits to the training path, no
Rust changes, no `.so` rebuild). Ownership is disjoint so parallel implementation does not conflict.
...
> modules is `packages/hexfield/python/hexfield/`, not `scripts/`; a thin standalone CLI under
> `scripts/` simply imports them.

| File | Status | Contents |
|---|---|---|
| `docs/specs/hexfield_eval_v2_spec.md` | **new (this doc)** | this spec |
| `packages/hexfield/python/hexfield/eval_stats.py` | **EXISTS — extend, do not clobber** | pure-numpy statistics core: converged BT (Newton/scipy + `max|grad|<1e-6` assert + full-Hessian covariance), pair-level SE, pentanomial summary, Wilson CI, SPRT(pairs), `var_diff` difference-CI with `Cov_LB`, `overdispersion_weight`, `effective_counts`, `expected_se_elo`. **Pure CPU/numpy (+optional scipy) → unit-testable without GPU.** |
| `packages/hexfield/python/hexfield/eval_arena.py` | **EXISTS — extend, do not clobber** | game-running layer: `play_checkpoint_match` (reuses `_play_pair`, CRN-paired seat-swapped games) and `play_sealbot_match` (concurrent multi-root port of the dense SealBot loop). Returns the arena result-dict shape + a pentanomial block. |
| `packages/hexfield/python/hexfield/eval_pool.py` (or in `eval_stats`/`eval_arena`) | **new/verify** | opponent-set construction (SealBot config, permanent anchors, sliding bracket on `{5,10,20,40,80,160}`, champion registry) + `eval_pool.json` load/merge/persist |
| `scripts/_hexfield_eval_v2.py` | **new/verify** | thin standalone multi-stage runner/CLI (Stages A–D orchestration) that imports the package modules above; off the training critical path |
| `tests/test_hexfield_eval_v2_stats.py` | **new/verify** | numpy-only unit tests: BT convergence (`max|grad|<1e-6` on a known-answer ladder), pairing widens CI vs naive binomial, pentanomial variance ≤ binomial, Wilson matches the arena helper, SPRT decision boundaries, difference-CI `Cov_LB` sign, no-write purely-eval sentinel |
| `diagnostics/eval_pool.json` (per run dir) | **runtime artifact** | persisted rolling BT pool (all historical edges as effective counts + champion registry) |

Reuse (read-only imports, no edits to these):
- `packages/hexfield/python/hexfield/evaluation.py` — `_play_pair` (per-seat overrides) + the
...
3. **Gating/promotion hooks exist but default OFF and are wired to nothing.** Two config flags are
   added to `EvaluationSection` (`packages/hexfield/python/hexfield/config.py:76`,
...
4. **The pipeline eval is untouched.** `hexfield/evaluation.py:evaluate_epoch` keeps running its
...
Pure-CPU/numpy unit tests (`tests/test_hexfield_eval_v2_stats.py`), runnable without GPU:
...
5. **Wilson parity:** matches `_wf_h2h2_arena.py:wilson_ci` to machine precision.
