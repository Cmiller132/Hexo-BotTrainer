...
- `training_progress` (optional): `epoch`, `status`, `progress` (0..1), `steps`,
  ...
  `dense_cnn.training_progress.epoch_*.json` as "no current producer emits this file"
...
- `selfplay`: `status, games, completed_games, truncated_games, samples_added,
  ...
  buffer`. Any of these may be null (dense_cnn lengths/fractions are backfilled
  server-side; nulls remain possible) — every chart/cell skips null points.
- `selfplay.buffer` (nested object or null): present for hexgt/hexgnn (bridge) AND
  synthesized for dense_cnn from training loss. Keys consumed today by
  ...
  loss_stvalue_<h>, optimism_sum_mean`. The synthesized dense_cnn buffer carries ONLY
  the `loss_*` keys (no pool/optimism), so guard every key individually.
- `training`: `status, loss, loss_components ({policy,value,opp_policy,stvalue_*}),
  ...
  charting come from `row.selfplay.buffer.loss_policy / .loss_value` (uniform across
  lineages — for dense_cnn they are synthesized there from `loss_components`). Do NOT
...
H3. Trends grid `renderHistTrends(runs)` writing `#histTrends`.
  ...
  - Charts (each rendered only when its series has >= 2 non-null points; otherwise
    ...
    - T6 Buffer (only when >= 2 epochs have a real pool, i.e.
      ...
      a second chart for `buffer.optimism_sum_mean` when >= 2 points. (hexgt runs
      only; dense runs simply lack these.)
...
H6. Epoch table `renderHistEpochTable(runs)` (replace the H1 stub) writing
  ...
  - Chevron `<button>` (`data-hist-epoch-toggle="<run>::<epoch>"`, 44px target)
    ...
    `epochProgressDetail(selfplay.buffer)` band (unchanged function). Dense runs
    whose buffer is loss-only show just the Losses group (epochProgressDetail's
...
A4. Epoch table: last 15 epochs, one row each, live row accented; expanding shows
    buffer/losses/calibration chips on hexgt runs and breaks nothing on dense runs.
...
- `training_progress` and `selfplay.buffer` pool stats are OPTIONAL per lineage; every
  region must render with a dense_cnn run (no buffer pool, no progress file), a hexgt
  run (full buffer), and a stopped run (no live status) without throwing.
