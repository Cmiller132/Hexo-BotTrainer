...
# x A/B x cap 10k under the official 1 GiB profile. Rescope rationale in
...
while tasklist //FI "IMAGENAME eq cargo.exe" 2>/dev/null | grep -qi "cargo.exe"; do
  echo "$(date -Is) waiting: cargo busy" >> ROOT_STABILIZER_ATLAS_10K_GATE.log
  sleep 300
done
...
echo "$(date -Is) gate free=$FREE standby=$STANDBY avail=$AVAIL req=12/6" >> ROOT_STABILIZER_ATLAS_10K_GATE.log
awk -v f="$FREE" -v a="$AVAIL" 'BEGIN{exit !(f>=6 && a>=12)}' || { echo "RAM gate FAILED" >> ROOT_STABILIZER_ATLAS_10K_GATE.log; exit 2; }
...
export TSS_ROOT_STABILIZER_FAMILIES=4
export TSS_ROOT_STABILIZER_TRANSFORMS=4
export TSS_ROOT_STABILIZER_CAPS=10000

cargo test --release --target x86_64-pc-windows-msvc -p hexfield_eq tss_root_stabilizer_atlas_campaign -- --ignored --test-threads=1 --nocapture > ROOT_STABILIZER_ATLAS_10K_RAW.log 2>&1
echo "SIZING_JOB_EXIT=$?" >> ROOT_STABILIZER_ATLAS_10K_RAW.log
