...
same microstep. Let `A_FS2^ET(sigma)` be the **terminal closure** of live
`A_FS2` trace segments whose
F-role steps are generated by Definition 46.1 and are event-terminal aligned.
On the final paired F microstep, Definition 46.1 replaces round-5
Definition 38.2 clauses 5-6: both associated physical appends — the real
F placement and its paired shadow `Fhat` placement — occur inside that one
coupled event before the trace closes, so a winning real append does not
retroactively forbid its already-paired shadow append, and clause 6's
nonwinning requirement applies only to non-final paired events (review
round-6 Finding 8).
...
|---:|---|---|
| 1 | Per-pair and broader branch-(A) repair | **PARTIAL.** S43 proves that lifetime counting cannot cross a pair reset. S43.1 refutes per-pair `K=2` when `T` is fixed within the pair. Intra-episode total rebinding, total nonisometric zero-lag point recodings (Definition 47.1's grammar is translation/D6-isometric, so these are a separately surviving class), non-total/window recoding, and one repair per placement indefinitely remain **OPEN**. |
...
| 10 | Window-certificate maintenance | **PARTIAL negative.** S44 applies even to a dynamically reselected Definition 39.1 selector at the post-F checkpoint. Newly admitted debt, physical reconciliation, phase-lagged events, broader certificates, common-only real wins, and simultaneous legality/terminal maintenance remain **OPEN**. |
...
| 2. P2/P4 at each real-S coordinate | **PARTIAL** | S34 remains positive on `A_FS2`; S43.1 refutes pair-static total-exact repair. General branch selection remains open. S13 (fixed-isometry one-stone FIFO frontier failure) remains a binding regression here; guarded `A_FS2` traces fall outside it via persistent genuine fillers/proxies and physical reconciliation. |
| 3. P5R during every lag/recode | **PROVEN in inherited conditional modules; OPEN globally** | `A_FS2` shielding and S38 certification remain sound. S44 prevents a deficit selector from hiding missed urgent service. S14 (unguarded literal one-S-stone-lag terminal-count failure) remains binding outside shield-admitted traces. |
...
## 53. Errata and clarifications folded from the round-6 hostile review

`STRATEGY_STEALING_REVIEW_ROUND6.md` (ultra, reviewed artifact `7c09dee4`,
authored at input `3000a117`) returned **SOUND-WITH-MINOR-ERRATA**: no
REFUTED or MAJOR finding; all three objectives CONFIRMED (S40-S42 at
conditional `A_FS2^ET` scope; per-pair K=2 partial; S44/S44.1/S45 with
exact `tau_E=5` — both the five-cell upper bound and the four-cell
impossibility independently recomputed). Folds:

1. **(Finding 8, MINOR)** `A_FS2^ET(sigma)` is now explicitly the
   *terminal closure* of live `A_FS2` segments, with Definition 46.1
   replacing round-5 clauses 5-6 on the final paired F microstep: both
   associated physical appends occur inside the one coupled event
   before the trace closes. This removes the literal
   real-service-then-shadow ordering ambiguity; S41's coordinates and
   terminal calculation are unchanged.
2. **(Finding 12, MINOR)** Ledger completeness restored in §50: obstacle
   row 1 now names total nonisometric zero-lag point recodings as a
   separately surviving open class; row 10 restores common-only real
   wins and simultaneous legality/terminal maintenance; agenda row 2
   restores the S13 regression caveat and row 3 the S14 caveat, each
   with the reason guarded `A_FS2` traces fall outside them.

**Review confirmations of record.** S40's event selector is causal
including the second prescription (Finding 6); S18 independence is real
and P5 transfer physical-but-conditional (Finding 7); S41 is a legal,
fresh, cadence-exact handler trace with exact inverse failures and
simultaneous proxy-assisted windows (Findings 9-10); S42 reaches its
obstruction on the named selector's own history (Finding 11); S43's
reset escape and S43.1's colored case split are exhaustive and
candidate-own (Findings 2-3); S44/S44.1 close at fixed debt and S45's
seven-window enumeration gives exact `tau_E=5` (Findings 4-5). The
review's updated unresolved-obstacle list supersedes §50.1 as the
