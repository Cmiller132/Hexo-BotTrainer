//! hexgnn MCTS Python boundary (tree + session copied verbatim from dense_cnn —
//! the search algorithm is model-agnostic; only `mcts_eval` differs, building a
//! graph payload + per-candidate priors instead of dense planes).
//!
//! Python hands live engine states to this module. The state bridge clones those
//! states into authoritative Rust `HexoState` values; from there the search never
//! mutates the live Python game. Tree mechanics live in `mcts_tree`, and evaluator
//! payload construction + parsing lives in `mcts_eval`.
//!
//! `HexgnnMctsSession` is intentionally stateful. A caller provides a stable
...
use hexo_engine::{apply_placement, unpack_coord, HexoState as RustHexoState, PackedCoord, Placement};
...
#[pyclass(unsendable)]
pub(crate) struct HexgnnMctsSession {
    ...
    // Candidate-set radius `n` (the move vocabulary). Immutable per session so the
    // transposition cache (keyed on state hash, which does NOT include `n`) stays
    // sound — every cached evaluation was produced at this same `n`.
    candidate_radius: i16,
}

#[pymethods]
impl HexgnnMctsSession {
    #[new]
    #[pyo3(signature = (max_states=None, n=None))]
    fn new(max_states: Option<usize>, n: Option<i16>) -> PyResult<Self> {
        let cache_max_states = validate_positive_usize(
            "max_states",
            max_states.unwrap_or(HEXGT_EVAL_CACHE_MAX_STATES),
        )?;
        let candidate_radius = n.unwrap_or(HEXGT_DEFAULT_CANDIDATE_RADIUS);
        if candidate_radius < 1 {
            return Err(PyValueError::new_err("candidate radius n must be >= 1"));
        }
        Ok(Self {
    ...
    fn search(
        ...
        if roots.len() != game_keys.len() {
            return Err(PyValueError::new_err(format!(
                "hexgnn MCTS session received {} game keys for {} states",
                game_keys.len(),
                roots.len()
            )));
        }
        ...
        let root_limit = validate_positive_usize(
            "active_root_limit",
            active_root_limit.unwrap_or(HEXGT_ACTIVE_ROOT_LIMIT),
        )?;
        if roots.len() > root_limit {
            return Err(PyValueError::new_err(format!(
                "hexgnn MCTS session received {} active roots, above strict limit {}",
                roots.len(),
                root_limit
            )));
        }
...
fn build_search_result_payloads(
    ...
    for (index, search) in searches.iter().enumerate() {
        ...
        // Tactical-safety guard (owner feedback item 2): identical inputs +
        // determinism as `select_search_action`, so the reported action matches the
        // one used to advance the native root.
        let guarded_weights =
...
/// Tactical-safety classification of a candidate ROOT move (owner feedback item
/// 2): `+1` if playing it is a proven 1-ply WIN for the side to move, `-1` if it
/// is a proven 1-ply LOSS (an immediate terminal loss, or it leaves the opponent
/// a forced win), else `0`. Exact and cheap — one placement + one phase-aware
/// threat analysis — matching the depth-1 scope of the leaf override (Phase 5).
...
/// PLAYED move (owner feedback item 2). Temperature/Dirichlet visit sampling alone
/// can select a move the search has effectively proven losing (forced playouts
/// keep a loser's visit count nonzero) or miss a proven win; this guard ensures a
/// proven win is played and a proven-losing move is never selected when a
/// non-losing move exists. It masks ONLY the selection weights — the exported
/// training policy is left untouched (the design doc's "don't collapse the policy
/// target"). Gated on the root carrying a threat / own win, so the quiet common
/// case is a no-op. Deterministic in (root_state, action_ids, weights), so both
/// selection sites (advance + result payload) stay identical.
...
pub fn register_pybridge(module: &Bound<'_, PyModule>) -> PyResult<()> {
    module.add_class::<HexgnnMctsSession>()?;
    Ok(())
}
