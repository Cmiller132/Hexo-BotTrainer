"""Losses and value-bin helpers for hexgnn (Model 2).
...
def hexgnn_loss(
    ...
    *,
    ...
    """Compute the weighted loss surface expected by `HexgnnTrainer`.

    `batch` carries `candidate_graph` (segment ids), `num_graphs`, the
    per-candidate `policy`/`opp_policy` targets, and the per-graph `value` target.
    hexgnn has only policy + value + opponent-policy heads (no STV lookahead).
    """
