"""Configuration objects for hexgnn (Model 2).

`parse_hexgnn_config` is the TOML boundary for this model. It mirrors dense_cnn's
...
class HexgnnArchitectureConfig:
    """Dynamic GNN-trunk hyperparameters (NO transformer) and the candidate rule `n`."""

    node_feature_dim: int = NODE_FEATURE_DIM
    token_dim: int = DEFAULT_TOKEN_DIM
    gnn_layers: int = DEFAULT_GNN_LAYERS
    # `attention_heads` is for the multi-head PMA value pool (hexgnn has NO
    # transformer, so there are no ctx_layers / ffn_dim, and no STV horizons).
...
@dataclass(frozen=True, slots=True)
class HexgnnConfig:
    architecture: HexgnnArchitectureConfig = field(default_factory=HexgnnArchitectureConfig)
    training: HexgnnTrainingConfig = field(default_factory=HexgnnTrainingConfig)
    samples: HexgnnSampleConfig = field(default_factory=HexgnnSampleConfig)
    selfplay: HexgnnSelfPlayConfig = field(default_factory=HexgnnSelfPlayConfig)
    evaluation: HexgnnEvalConfig = field(default_factory=HexgnnEvalConfig)
    performance: HexgnnPerformanceConfig = field(default_factory=HexgnnPerformanceConfig)
    device: str = "cuda"
    checkpoint_path: Path | None = None


def parse_hexgnn_config(raw: Mapping[str, Any] | None) -> HexgnnConfig:
    """Parse the hexgnn model section into immutable config dataclasses."""
    ...
    arch = _section(config, "architecture", HexgnnArchitectureConfig)
    training = _section(config, "training", HexgnnTrainingConfig)
    samples = _section(config, "samples", HexgnnSampleConfig)
    selfplay = _section(config, "selfplay", HexgnnSelfPlayConfig)
    evaluation = _section(config, "evaluation", HexgnnEvalConfig)
    performance = _section(config, "performance", HexgnnPerformanceConfig)

    checkpoint_path = config.get("checkpoint_path")
    return HexgnnConfig(
        architecture=HexgnnArchitectureConfig(
        ...
        performance=HexgnnPerformanceConfig(
