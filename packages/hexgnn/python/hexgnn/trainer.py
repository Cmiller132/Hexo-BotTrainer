"""HexgnnTrainer — training loop with dense_cnn discipline for the graph model.
...
the dynamic per-graph policy CE (`losses.hexgnn_loss`). Inputs are recomputed at
...
from .losses import hexgnn_loss
...
class HexgnnTrainState:
    ...
    @classmethod
    def from_mapping(cls, state: Mapping[str, Any] | None) -> "HexgnnTrainState":
        if not state:
            return cls()
        return cls(
...
class HexgnnTrainer:
    def __init__(self, *, model: torch.nn.Module, config: HexgnnConfig, optimizer: torch.optim.Optimizer) -> None:
        self.model = model
        self.config = config
        self.optimizer = optimizer
        self.train_state = HexgnnTrainState()
    ...
    def load_train_state(self, state: Mapping[str, Any] | None) -> None:
        self.train_state = HexgnnTrainState.from_mapping(state)
        ...
        for attempt in range(attempts):
            try:
                ...
                with torch.autocast(device_type=device.type, enabled=use_amp):
                    ...
                    loss, components = hexgnn_loss(outputs, targets, **self._weights)
