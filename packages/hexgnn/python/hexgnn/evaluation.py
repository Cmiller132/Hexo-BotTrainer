"""Head-to-head evaluation for hexgnn (Model 2).
...
- `run_head_to_head` — the reusable, deterministic match driver. Pairs any two
  ...
  standalone hexgnn-vs-dense_cnn / hexgnn-vs-SealBot comparisons (matched-compute
...
from .player import HexgnnPlayer
...
class HexgnnBatchedSearcher:
    ...
    def __init__(self, model: Any, config: HexgnnConfig, *, device: str, fp16: bool, visits: int,
        ...
        self.inference = HexgnnInference(model, device=device, fp16=fp16)
...
def make_hexgnn_factory(
    model: Any,
    config: HexgnnConfig,
    *,
    device: str,
    fp16: bool = True,
    identity_id: str = "hexgnn",
    opening_temperature: float = 0.0,
    opening_moves: int = 0,
    virtual_batch_size: int = 0,
) -> PlayerFactory:
    """A `run_head_to_head` factory that builds a deterministic `HexgnnPlayer`."""

    def factory(seed: int) -> HexgnnPlayer:
        return HexgnnPlayer(
...
def evaluate_epoch(*, ctx: Any, components: Any, epoch: int) -> dict[str, Any]:
    ...
    try:
        ...
        path = ctx.diagnostics.write_json(f"hexgnn.evaluation.epoch_{epoch:06d}.json", payload)
    ...
    make_hexgnn = make_hexgnn_factory(
        model,
        config,
        device=device,
        fp16=(device != "cpu"),
        identity_id="hexgnn-eval",
    ...
    path = ctx.diagnostics.write_json(f"hexgnn.evaluation.epoch_{epoch:06d}.json", diagnostics)
