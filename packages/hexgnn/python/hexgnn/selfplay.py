"""Game-driven self-play sample generation for hexgnn (Model 2).
...
dynamic-GNN `HexgnnMctsSession` + `HexgnnInference` evaluator and writes the SAME
...
(`HexgnnTrainer.train_on_shards` -> `expand.build_training_batch`) consumes those
...
`HexgnnMctsSession.run` already coalesces the in-flight leaves of *all* concurrent
...
class SelfPlayResult:
    ...
    # Non-finite-logit sanitization audit (see HexgnnInference._count_and_sanitize).
...
_RECORD_PLAYERS = (
    HexoRecordPlayer("hexgnn-a", "player0", "hexgnn A"),
    HexoRecordPlayer("hexgnn-b", "player1", "hexgnn B"),
)
    ...
    if truncated:
        writer.finish_aborted(
            AbortRecord(
                ...
                message=f"hexgnn self-play reached max_actions={max_actions}",
            )
        )
    else:
...
def run_selfplay_games(
    ...
    *,
    ...
    # hexgnn has NO short-term-value heads. The shared dense_cnn finalize/write
    # API still takes a horizons arg; () means "no STV targets" (the per-ply STV
    # EMA chain is simply not constructed), leaving policy/value/opp shards intact.
    horizons: tuple[int, ...] = ()
    ...
    inference = HexgnnInference(model, device=device, fp16=fp16)
    ...
    abs_value_sum = 0.0
    abs_value_n = 0
    # Sanitization-audit accumulators (see HexgnnInference._count_and_sanitize).
        ...
        if playable:
            ...
            for spec_index, (spec_games, is_full) in enumerate(search_specs):
                ...
                if len(searches) != len(spec_games):
                    raise RuntimeError(
                        f"hexgnn MCTS returned {len(searches)} results for {len(spec_games)} playable games"
                    )
...
def generate_selfplay_epoch(
    *, ctx: Any, components: Any, epoch: int, games_per_epoch: int
    ...
    """Plugin hook: one epoch of hexgnn self-play, writing per-game compact shards.
    ...
    config: HexgnnConfig = trainer.config
    ...
    if hasattr(ctx, "diagnostics"):
        ctx.diagnostics.write_json(f"hexgnn.selfplay.epoch_{epoch:06d}.json", summary)
    return summary
