"""Torch (FP16) inference for hexgnn — the search evaluator path (Phase 5).
...
from .architecture import precompute_side_rows
...
# Peak forward VRAM is ~linear in this product. A LARGER budget means fewer, fatter
...
# HEXGNN_FORWARD_PAD_BUDGET env var (the run launcher sets it to exploit headroom).
# (hexgnn has no transformer, so the chunking is purely a GNN/PMA forward-VRAM cap;
# graphs are independent, so the reassembled output is bit-identical to one forward.)
...
class HexgnnInference:
    ...
    def __init__(
        ...
        *,
        ...
        # Resolve the chunk pad-budget: explicit arg > HEXGNN_FORWARD_PAD_BUDGET env
        ...
        if forward_pad_budget is None:
            env_budget = os.environ.get("HEXGNN_FORWARD_PAD_BUDGET")
        ...
        env_pin = os.environ.get("HEXGNN_PIN_HOST")
    ...
    def forward_batch(self, batch: dict) -> tuple[dict[str, torch.Tensor], dict]:
        ...
        order, and ``dev_batch`` is the on-device batch (with SIDE-hub rows) the
        ...
        # data-dependent nonzero/.item -> no graph-break / recompile. (hexgnn has no
        # transformer, so there is no padded attention layout to precompute.)
        with torch.autocast(device_type=self.device.type, dtype=torch.float16, enabled=self.fp16):
            if num_graphs <= 1:
                batch = {**batch, **precompute_side_rows(batch["node_type"], batch["node_graph"])}
            ...
            for gids in chunks:
                ...
                sub = {**sub, **precompute_side_rows(sub["node_type"], sub["node_graph"])}
