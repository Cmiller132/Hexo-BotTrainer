"""Checkpoint IO for hexgnn training state.

Mirrors dense_cnn's checkpoint discipline (`dense_cnn/checkpoints.py`): the
generic pipeline owns *when* checkpoints load/save; this module owns *what* hexgnn
...
class HexgnnCheckpointLoader:
    """Load hexgnn checkpoint payloads into generic pipeline components."""
    ...
    def load(self, checkpoint_ref: object | None, *, ctx: Any, components: Any) -> dict[str, Any]:
        ...
        if incompatibilities:
            return {
                "status": "initialized",
                "checkpoint_ref": str(path),
                "reason": "checkpoint model_state is incompatible with current hexgnn architecture",
...
class HexgnnCheckpointSaver:
    """Save hexgnn checkpoint payloads from generic pipeline components."""
