...
| `rust/src/lib.rs` | Crate root. `#[path]`-includes the lineage Rust trees (below), registers them as submodules `hexo_models._rust.{dense_cnn,hexgt}`, and patches each into `sys.modules` so `import hexo_models._rust.<lineage>` resolves. |
| `rust/src/threats_shared.rs` | Shared TSS core (threat / win-now / forced-loss analysis over the engine `WindowStore`), reached as `crate::threats_shared` by the dense_cnn and hexgt Rust. |
...
(A third lineage, **hexgnn** — a parked fork of hexgt that compiled into this
crate as `hexo_models._rust.hexgnn` — has been removed from the repo.)

## Lineage status

| Lineage | Python status | Rust status |
|---|---|---|
| `dense_cnn` | **Legacy** as a training line (superseded by `packages/dense_cnn_restnet`), still loadable: old checkpoints, ~14 test files, the dashboard debug worker, and scripts remain consumers. | **Active production.** The live `dense_cnn_restnet` runs (main_3/main_4) ship no Rust of their own and drive `hexo_models._rust.dense_cnn` (encoding, MCTS incl. the continuous scheduler, sample facts) read-only. |
| `hexgt` | **Halted lineage** (run `hexgt_rl_main3` halted at epoch 40, 2026-06-05 per `HANDOFF.md`). Live infrastructure remains: the frontend debug worker loads hexgt checkpoints, ~30 test files gate Rust/Python parity. | Compiled into every build; used by tests. |
...
- `from hexo_models import _rust` then `_rust.dense_cnn` / `_rust.hexgt`
  -- via each lineage's `rust_bridge.py` (including
