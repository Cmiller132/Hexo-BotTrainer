//! hexgt MCTS Python boundary (tree + session copied verbatim from dense_cnn —
...
//!
//! `HexgtMctsSession` is intentionally stateful. A caller provides a stable
...
#[pyclass(unsendable)]
pub(crate) struct HexgtMctsSession {
...
pub fn register_pybridge(module: &Bound<'_, PyModule>) -> PyResult<()> {
    module.add_class::<HexgtMctsSession>()?;
    Ok(())
}
