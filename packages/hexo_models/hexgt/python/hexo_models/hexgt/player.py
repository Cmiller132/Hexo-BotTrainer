"""`hexo_runner` player adapter for hexgt (Model 2) inference + search.

Mirrors `dense_cnn/player.py`: the runner speaks generic player-lifecycle
methods over live engine states; this adapter wraps a `HexgtInference` evaluator
and one persistent native `HexgtMctsSession` per game, searches the current live
...
class HexgtPlayer:
    """Runner-compatible player backed by the hexgt dynamic-GNN MCTS."""
    ...
    inference: HexgtInference = field(init=False)
    mcts_session: HexgtMctsSession = field(init=False)
    _decisions_made: int = field(init=False, default=0)

    def __post_init__(self) -> None:
        self.identity = PlayerIdentity(self.identity_id, label="hexgt")
        self.inference = HexgtInference(self.model, device=self.device, fp16=self.fp16)
