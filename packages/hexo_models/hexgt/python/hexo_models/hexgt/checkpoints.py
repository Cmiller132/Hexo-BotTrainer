"""Checkpoint IO for hexgt training state.

Mirrors dense_cnn's checkpoint discipline (`dense_cnn/checkpoints.py`): the
generic pipeline owns *when* checkpoints load/save; this module owns *what* hexgt
persists — model weights, optimizer state, and KataGo-style train-bucket state.

Loading is strict about model-weight compatibility and rejects legacy
checkpoints that still carry the removed in-memory replay buffer, so hexgt has
...
MODEL_NAME = "hexo_models.hexgt"


class HexgtCheckpointLoader:
    """Load hexgt checkpoint payloads into generic pipeline components."""
    ...
    def load(self, checkpoint_ref: object | None, *, ctx: Any, components: Any) -> dict[str, Any]:
        ...
        if payload.get("sample_buffer") is not None:
            return {
                "status": "initialized",
                "checkpoint_ref": str(path),
                "reason": "legacy sample_buffer checkpoints are unsupported",
        ...
        if incompatibilities:
            return {
                "status": "initialized",
                "checkpoint_ref": str(path),
                "reason": "checkpoint model_state is incompatible with current hexgt architecture",
...
class HexgtCheckpointSaver:
    """Save hexgt checkpoint payloads from generic pipeline components."""
