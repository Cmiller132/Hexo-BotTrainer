//! Dense CNN Model1 Rust acceleration.
...
use hexo_engine::{
    apply_placement, pack_coord, unpack_coord, Axis, GameOutcome, HexCoord,
    HexoState as RustHexoState, MoveError, Placement, Player, TurnPhase,
};
...
fn states_from_history_rows(history_rows: &Bound<'_, PyAny>) -> PyResult<Vec<RustHexoState>> {
...
fn state_from_history_row(row: &Bound<'_, PyAny>) -> PyResult<RustHexoState> {
    let mut state = RustHexoState::new();
    for item in row.try_iter()? {
        let action_id = item?.extract::<u32>()?;
        apply_placement(
            &mut state,
            Placement {
