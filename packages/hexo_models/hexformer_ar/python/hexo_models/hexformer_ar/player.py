"""Runner player adapter for Hexformer AR."""
...
class HexformerPlayer:
    identity_id: str
    model: Any
    trainer: Any
    identity: PlayerIdentity = field(init=False)
    inference: HexformerInference = field(init=False)

    def __post_init__(self) -> None:
        self.identity = PlayerIdentity(self.identity_id, label="Hexformer AR")
        self.inference = HexformerInference(
            self.model,
    ...
    def decide(self, state: object) -> DecisionResult:
        search = run_mcts(
            ...
            c_puct=self.trainer.config.selfplay.c_puct,
            temperature=0.0,
        )
