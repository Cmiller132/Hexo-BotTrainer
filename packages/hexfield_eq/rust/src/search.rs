...
#[pyfunction]
#[pyo3(signature = (node_cap, horizon, ladder, zone, wide, dual_pass=false, loss_reserve_nodes=0, group2=false))]
...
pub fn hexfield_eq_deep_solve_probe(
    ...
    let mut solver = harness_solver(wide, zone_caps, false, 0, false);
    ...
    if with_stats {
        let mut stats_solver = harness_solver(wide, zone_caps, false, 0, false);
...
#[pyfunction]
#[pyo3(signature = (states, node_cap, goal, horizon, ladder, zone, wide, dual_pass=false, loss_reserve_nodes=0, group2=false))]
#[allow(clippy::too_many_arguments)]
pub fn hexfield_eq_deep_solve_batch(
    ...
    let zone_caps = harness_zone_caps(zone);
    let mut solver = harness_solver(wide, zone_caps, dual_pass, loss_reserve_nodes, group2);
...
mod fallback_tests {
    ...
    fn solver_manifest_matches_real_solve_effective_config() {
        ...
        let manifest = harness_solver_manifest(500, horizon, false, true, false, 0, false);
        let state = RustHexoState::new();
        let zone = harness_zone_caps(false);
        let mut solver = harness_solver(true, zone, false, 0, false);
        let mut counters = TssCounters::default();
    ...
    fn solver_manifest_echoes_dual_pass_from_shared_resolver() {
        ...
        for dual_pass in [false, true] {
            let manifest =
                harness_solver_manifest(500, horizon, false, true, dual_pass, 0, false);
            assert_eq!(manifest.effective.dual_pass, dual_pass);

            let solver = harness_solver(true, harness_zone_caps(false), dual_pass, 0, false);
            ...
            Python::attach(|py| {
                let echoed = hexfield_eq_solver_manifest(
                    py, 500, 0, false, false, true, dual_pass, 0, false,
                )
                .unwrap();
    ...
    fn solver_manifest_echoes_group2_from_shared_resolver() {
        ...
        for group2 in [false, true] {
            let manifest = harness_solver_manifest(500, horizon, false, true, false, 0, group2);
            assert_eq!(manifest.effective.group2, group2);

            let solver = harness_solver(true, harness_zone_caps(false), false, 0, group2);
            let caps = tss_verified_solve_caps(0, 500, horizon);
            assert_eq!(
                solver.effective_solve_config(&caps, solver.sample_runtime_flags()),
                manifest.effective,
            ...
            Python::attach(|py| {
                let echoed = hexfield_eq_solver_manifest(
                    py, 500, 0, false, false, true, false, 0, group2,
                )
                .unwrap();
                let dict = echoed.bind(py).cast::<PyDict>().unwrap();
                assert_eq!(
    ...
    fn persistent_batch_stats_report_warm_fragment_imports_only_when_enabled() {
        ...
        env.set(Some("1"));
        let mut warm_solver = harness_solver(true, zone, false, 0, false);
        ...
        env.set(None);
        let mut cold_solver = harness_solver(true, zone, false, 0, false);
