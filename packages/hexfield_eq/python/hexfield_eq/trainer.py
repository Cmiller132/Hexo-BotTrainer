...
# One-time raylen sanity latch (spec D-S31): an 'L' layout with ray blockers on
# must see nonzero raylen over live cells in its first training batch — an
# all-zero tensor means stale shards or an old _rust .so and would silently
# kill every ray.
_RAYLEN_SANITY_DONE = False


def _check_raylen_once(model, batch: dict) -> None:
    global _RAYLEN_SANITY_DONE
...
class HexfieldTrainer:
    ...
    def _get_expand_pool(self):
        ...
        the parent environment, so each worker re-reads
        ``HEXFIELD_EQ_SUPPORT_RADIUS`` at ``support`` import time.
        """
    ...
    def _build_grad_norm_groups(self) -> dict[str, list[torch.nn.Parameter]]:
        """Partition model params into trunk_conv / trunk_attn / trunk_reg / heads.

        Used for per-group pre-clip grad-norm logging. ``stem*`` / ``conv_blocks*``
        -> trunk_conv; ``attn_blocks*``, ``tokens``, ``bias_free_tables*`` (the
        orbit-tied bias free tables), ``bias_theta*`` (the joint-tied A- and
        L-block thetas), and the Phase-L1 ray blocks (``ray_blocks*`` /
        ``ray_bias_free_tables*``) -> trunk_attn; ``registers*`` /
        ``tok_reads*`` (the Phase-R0 register lane, watched as its own signal
        during grow-in) -> trunk_reg; everything else -> heads.
        """
        ...
        for name, p in self.model.named_parameters():
            if name.startswith("stem") or name.startswith("conv_blocks"):
                ...
                or name == "tokens"
                or name.startswith("bias_free_tables")
                or name.startswith("bias_theta")
                or name.startswith("ray_blocks")
                or name.startswith("ray_bias_free_tables")
            ):
                groups["trunk_attn"].append(p)
            elif name.startswith("registers") or name.startswith("tok_reads"):
                groups["trunk_reg"].append(p)
            else:
    ...
    def train_passes(self, *, passes, sample_window, sample_symmetries, ctx, components, epoch) -> dict[str, Any]:
        ...
        # When HEXFIELD_EQ_SUPPORT_RADIUS < 8, tolerate (flag invalid, skip) replay
        ...
        for expanded in _iter_nominal_batches():
            ...
            for bucket in pair_budget_microbuckets(
                ...
                _check_raylen_once(self.model, batch)
                with torch.autocast(
        ...
        result = {
            ...
            **{
                ...
                for g in ("trunk_conv", "trunk_attn", "trunk_reg", "heads")
            },
