"""Multi-stage hexfield strength evaluation orchestrator.

Wires the game-running layer (:mod:`hexfield.eval_arena`) to the statistics
layer (:mod:`hexfield.eval_stats`) into a staged protocol. Measures the
...
def _live_featurize_radius() -> int:
    ...
    ``HEXFIELD_EQ_SUPPORT_RADIUS`` (support._SUPPORT_RADIUS); the in-process eval
...
def _resolve_anchor_path(run_dir: Path, checkpoints_dir: Path, raw: str) -> Path:
    ...
    slash (e.g. ``runs/hexfield_bc_1/checkpoint_epoch2.pt``) is resolved against
    ...
    if "/" in raw:
        ...
        head = raw.split("/", 1)[0]
        roots: list[Path] = []
        # (0) Env-overridable extra search roots: HEXFIELD_ANCHOR_ROOTS
        ...
        #     <repo>/packages/hexfield/python/hexfield/multistage_eval.py, so
        ...
        for multi_attempts in range(1, attempts + 1):
            try:
                matches = play_multi_checkpoint_match(
                    ...
                    # Epoch-folded CRN base so successive epochs play
                    # decorrelated games (seed base 0 every epoch replayed the
                    # same streams).
                    game_seed_base=_epoch_seed_base(roster.candidate_epoch),
                )
                multi_error = None
