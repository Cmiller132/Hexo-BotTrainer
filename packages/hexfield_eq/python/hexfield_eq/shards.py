...
enum. The graded per-axis window planes are recomputed from the placement
history at expand time, so no hot/standing-win cell columns are stored.
...
the stored legal_ids and derives legality from stones.
"""
...
# visit target.
#
# v4 (hexfield_eq): retires the four hot/standing-win cell CSR columns
# (own_hot/opp_hot/own_win/opp_win). The graded per-axis window planes are
