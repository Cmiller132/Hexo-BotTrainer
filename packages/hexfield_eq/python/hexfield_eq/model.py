...
class HexNodeConv(nn.Module):
    ...
    def _init_raytap(self) -> None:
        ...
        if self.equivariant and not getattr(self, "_pure_regular_in", True):
            # Typed-alpha (per-type-block reach profiles + a re-derived own/opp
            # visibility split for non-regular types) is NOT implemented yet:
            # alpha's orbit indexing and the visibility-half split below assume
            # a pure regular input fiber. Reject loudly rather than silently
            # mis-typing the aggregate (integration follow-up).
            raise NotImplementedError(
                "ray-tap on a mixed quotient input signature is not supported "
                "yet; use a pure-regular signature (TYPE_SIG unset) or "
                "raytap='0'"
            )
        corb_in = (
...
class ConvBlock(nn.Module):
    ...
    def __init__(self, channels: int, *, signature=None, raytap: str = "0") -> None:
        super().__init__()
...
class HexfieldNet(nn.Module):
    ...
    def __init__(
        ...
        if rt_mode != "0":
            if EQUIVARIANT and not (
                len(self._type_signature) == 1
                and self._type_signature[0][0] == "reg"
            ):
                # Typed-alpha not implemented (see HexNodeConv._init_raytap).
                raise NotImplementedError(
                    "raytap != '0' with a mixed quotient type signature is "
                    "not supported yet; unset HEXFIELD_EQ_TYPE_SIG or set "
        ...
        self.conv_blocks = nn.ModuleList(
            [
                ConvBlock(c, signature=self._type_signature, raytap=rt_mode)
