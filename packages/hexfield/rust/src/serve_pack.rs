...
// hexfield constants.py NUM_TOKENS (8).
const NUM_TOKENS: usize = 8;
// Planner defaults; mirror inference.py PAIR_CEILING / WASTE_FRACTION (env
// overrides below). NOTE: unlike hexfield_eq, this lineage's Python planner
// has NO MAX_GROUP_ROWS row cap, so none is applied here either — the Rust
// plan must stay byte-identical to the Python plan for the same inputs.
...
/// constants, same env overrides, same break order).
