...
fn append_history_hash(history_hash: StateHash, record: PlacementRecord) -> StateHash {
    mix_hash(
        history_hash
            ^ record_hash(record)
            ^ mix_hash(record.placement_index as u64 ^ 0x9e37_79b9),
    )
}
...
fn phase_hash(phase: TurnPhase) -> StateHash {
    match phase {
        ...
        TurnPhase::SecondStone { first } => {
            mix_hash(0x1319_8a2e_0370_7344 ^ coord_hash(first))
        }
    }
}
...
fn coord_hash(coord: HexCoord) -> StateHash {
    let q = coord.q as i32 as u64;
    let r = coord.r as i32 as u64;
    mix_hash(q.wrapping_mul(0xbf58_476d_1ce4_e5b9) ^ r.wrapping_mul(0x94d0_49bb_1331_11eb))
}
...
mod tests {
    ...
    fn hash_distinguishes_same_board_different_history_order() {
        ...
        let first_board: HashSet<_> =
            first_order.board().occupied_cells().iter().copied().collect();
        let second_board: HashSet<_> =
            second_order.board().occupied_cells().iter().copied().collect();

        assert_eq!(first_board, second_board);
        assert_ne!(first_order.placement_history(), second_order.placement_history());
