...
`packages/hexo_models/dense_cnn` ("Model 1") lineage; the `main1` / `main_2` /
`main_3` configs are earlier runs of the same lineage.

The package is **pure Python / PyTorch and carries no Rust of its own**. It reuses
the native accelerator `hexo_models._rust.dense_cnn` read-only (featurizer, batched
MCTS sessions including `run_continuous`, sample facts, the `(N, 13, 41, 41)` byte
contract). The Rust source lives in `packages/hexo_models/dense_cnn/rust/` and is
built by maturin into the `hexo_models` wheel: Rust changes take effect after
`scripts/_rebuild_hexo_models_hexgt.sh` runs in the WSL venv, and a single rebuild
serves both this lineage and the parent `dense_cnn` lineage. Internal symbols and
diagnostics filenames retain the `Model1` / `model1_*` / `dense_cnn.*` names of the
parent lineage.
...
| `architecture.py` | `RestnetNetwork`: hex-masked conv stem, residual/transformer interleaved trunk defined entirely by the `blocks_type` string, relative-position MHSA, heads_v3 dual `ValueReduction` (main value vs aux stvalue/moves_left). `optimized_restnet_for_inference` does inference-time folding. |
...
| `checkpoints.py` | Checkpoint loader/saver: `.pt` payload `{model: "dense_cnn_restnet", model_state, optimizer_state, train_state, epoch, metadata}`; fail-loud on shape mismatch at resume; `.txt` pointer indirection. `initialize_from` refs load weights only (fresh optimizer/train state), detected by comparing the incoming ref against `checkpoint.initialize_from`; any other ref is a full resume. |
...
| `player.py` | hexo_runner player adapter retained from the fork; production match play imports the parent lineage's `hexo_models.dense_cnn.player`. |
| `debug_artifacts.py` | Optional PNG game renderer retained from the fork; not wired into the pipeline. |
...
1. **Selfplay** (`selfplay.generate_selfplay_epoch`): runs many concurrent games
   ...
   *continuous* scheduler (config `selfplay.scheduler`). The Rust search calls
   ...
   Python returns value/prior bytes. Per-move temperature composes four schemes
   (linear decay, anchor schedule, adaptive halflife EMA, opening floor) plus
   the KataGo root-policy early ramp; precedence is defined by the branch order
   of `_move_temperature` and documented in `config.py`. The PCR full/fast coin
   and policy-init draws are deterministic per scheduler: lockstep uses a
   Python splitmix64 coin, continuous uses the native Rust `mix_seed`, so each
   scheduler defines its own schedule for a given seed. Finished games are
   finalized (`samples.finalize_game_samples`: z/soft-Z value targets,
   opp-policy, STV, moves_left) and written as one compact NPZ shard + JSON
   sidecar per game under `<run_dir>/selfplay/` (`replay.write_selfplay_npz`).
   The adaptive-temperature game-length EMA persists in
   `selfplay/length_ema.json`; it is seeded from `temperature_length_prior`
   only when the file is absent, after which the persisted value carries
   across restarts.
2. **Window selection** (`trainer.select_training_samples` ->
   ...
   most recent shards (mtime-ordered; window-seeding scripts preserve mtimes
   with `cp -p`), with policy-surprise frequency weighting, length-decay row
   drops, and an md5 train/validation split.
...
Diagnostics are written via `ctx.diagnostics` as `dense_cnn.selfplay.live.json`,
`dense_cnn.selfplay.epoch_*.json`, and `dense_cnn.evaluation.epoch_*.json`; the
`dense_cnn.` prefix is part of the consumer contract -- the dashboard and health
scripts key on these names.

## The frozen-win override (main_4, high level)

Design context: the network sees only a radius-20 hex-disk crop around the stone
centroid, and the Rust encoder *intentionally excludes out-of-crop legal moves*
from policy/MCTS. A game can therefore reach a state where a player has a
"standing win" (an empty cell completing 6-in-a-row) that lies outside the crop
and is invisible to both players' search; the game then continues past a won
position, distorting value targets. This mechanism was identified as the root
cause of the main_3 collapse and motivated the override introduced for main_4.

Mechanism (`selfplay.frozen_win_override` + `win_tracker.py`): during selfplay,
...
| Dependency | Used for |
|---|---|
| `hexo_models._rust.dense_cnn` (via `rust_bridge.py`) | Featurizer, MCTS sessions (`run` + `run_continuous`), sample facts. Read-only; the Rust source lives in `packages/hexo_models/dense_cnn/rust/`, built by maturin into the `hexo_models` wheel and shared with the parent lineage. |
...
Run directories live under `/mnt/e/Hexo-BotTrainer/runs/`, on a separate mount
from the repo checkout (`/mnt/e/Hexo-BotTrainer-hexgt`); the ops scripts above
