...
Request:  {"id": int, "op": "ping|info|analyze|search|reeval", ...}
Response: {"id": int, "ok": true, "result": {...}}  |  {"id": int, "ok": false, "error": str}

Ops:
  ping     -> {"pong": true}
  info     {checkpoint}            -> checkpoint provenance (graft, epoch, ...)
  analyze  {checkpoint, action_ids, n?} -> all model heads for the position
...
def _translate_path(path: str) -> str:
    ...
    ``E:\\Hexo-BotTrainer-hexgt\\runs\\x.pt`` must become
    ``/mnt/e/Hexo-BotTrainer-hexgt/runs/x.pt``. A path that is already POSIX is
    returned unchanged.
    """
...
def _handle(req: dict[str, Any]) -> dict[str, Any]:
    ...
    if op == "analyze":
        ...
        result = di.analyze_position(loaded, action_ids, n=req.get("n"))
