...
Read-only against the live ``hexfield_main_2`` samples tree. Two gates:

1. DECODE PARITY — for a sample of real shards, ``load_packed_shard(path)`` then
...
CPU only, no GPU, no model. Run:
  PYTHONPATH=packages/hexfield/python:packages/dense_cnn_restnet/python \
    python tests/katago_buffer/test_p3_window.py
"""
...
SAMPLES_ROOT = "/mnt/e/Hexo-BotTrainer/runs/hexfield_main_2/samples"


def _sample_shard_paths(limit: int) -> list[str]:
    """A spread of real shards across several epochs (read-only)."""
    paths: list[str] = []
    for epoch in (1, 2, 3, 5, 8, 13, 21):
        hits = sorted(glob.glob(f"{SAMPLES_ROOT}/epoch_{epoch:06d}/game_*.npz"))
        # take a few from the front and the back of each epoch dir
        for p in hits[:2] + hits[-1:]:
            if p not in paths:
                paths.append(p)
    if not paths:
        # fall back to whatever exists anywhere
        paths = sorted(glob.glob(f"{SAMPLES_ROOT}/epoch_*/game_*.npz"))
    return paths[:limit]
...
def test_decode_parity(paths: list[str]) -> int:
    total_rows = 0
    for p in paths:
...
def _assert_view_views_are_zero_copy(path: str) -> None:
...
def main() -> int:
    paths = _sample_shard_paths(limit=14)
    if not paths:
        print("FAIL: no real shards found under", SAMPLES_ROOT)
        return 1
    print(f"P3 window test — {len(paths)} real shards (read-only) from {SAMPLES_ROOT}")
    test_decode_parity(paths)
    _assert_view_views_are_zero_copy(paths[0])
    print("  zero-copy: row_view slices are numpy VIEWS (shared base)")
    test_concat(paths)
    test_empty_and_concat_with_empty(paths)
    print("PASS")
    return 0
