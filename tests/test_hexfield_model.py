...
def test_conv_oracle_against_dense_cnn_hexconv2d() -> None:
    ...
    with torch.no_grad():
        dense.weight.zero_()
        # tap 0 = center -> kernel (1,1); direction (dq,dr) -> kernel (dr+1, dq+1)
        dense.weight[:, :, 1, 1] = mine.weight[0].T
        for k, (dq, dr) in enumerate(C.DIRECTIONS):
            dense.weight[:, :, dr + 1, dq + 1] = mine.weight[k + 1].T
        dense.bias.copy_(mine.bias)
