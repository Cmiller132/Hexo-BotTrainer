"""Telemetry upgrade for the hexfield train/select path (diagnostics only).
...
2. The select diagnostic (``HexfieldTrainer.select_training_samples`` result +
   the ``hexfield.select.epoch_*.json`` it writes atomically) carries the new
...
fixture style mirrors tests/test_hexfield_durability.py (``_write_shard`` +
...
from hexfield.batching import policy_surprise_weights
...
from hexfield.window import build_window_split, load_packed_shard


# --- fixtures (mirrors tests/test_hexfield_durability.py) ---------------------


def _sample(turn: int) -> HexfieldSampleData:
...
class _TinyModel(torch.nn.Module):
    """Minimal nn.Module so HexfieldTrainer.__init__ can partition params via
...
def test_select_diag_carries_new_keys(tmp_path) -> None:
    ...
    hexfield.select.epoch_*.json both carry the new telemetry keys, without
    ...
    diag_path = diag_dir / "hexfield.select.epoch_000003.json"
    assert diag_path.exists()
    assert not (diag_dir / "hexfield.select.epoch_000003.json.tmp").exists()
...
# expansion). Reuse the same live/scratch shards the p5 e2e gate copies; skip
# cleanly when they are absent (e.g. a CI box without the live tree), exactly as
# tests/katago_buffer/test_p5_e2e.py depends on them.
_P5_SCRATCH_SAMPLES = (
    Path(__file__).resolve().parent
    / "katago_buffer" / "_scratch" / "p5" / "samples"
)
_LIVE_SAMPLES = Path("/mnt/e/Hexo-BotTrainer/runs/hexfield_main_2/samples")


def _real_shard_source() -> Path | None:
    for root in (_P5_SCRATCH_SAMPLES, _LIVE_SAMPLES):
        if root.exists() and any(root.glob("epoch_*/game_*.npz")):
            return root
    return None


def test_train_passes_writes_enriched_training_diag(tmp_path, capsys) -> None:
    """A real (tiny) HexfieldNet CPU train pass over real replay shards: the
    training diag json is written atomically and carries the new keys
    ...
    same select->train path as tests/katago_buffer/test_p5_e2e.py; skips when no
    real shards are available (synthetic rows are off-legal by design)."""
    from hexfield.model import HexfieldNet

    src = _real_shard_source()
    if src is None:
        pytest.skip("no real replay shards available (p5 scratch / live tree absent)")

    samples_dir = tmp_path / "samples"
    diag_dir = tmp_path / "diagnostics"
    diag_dir.mkdir(parents=True, exist_ok=True)
    # Copy a handful of real (npz+json) pairs from one epoch into the tmp buffer.
    import shutil

    copied = 0
    for npz in sorted(src.glob("epoch_*/game_*.npz"))[:6]:
        side = npz.with_suffix(".json")
        if not side.exists():
            continue
        dst_dir = samples_dir / npz.parent.name
        dst_dir.mkdir(parents=True, exist_ok=True)
        shutil.copy2(npz, dst_dir / npz.name)
        shutil.copy2(side, dst_dir / side.name)
        copied += 1
    if copied == 0:
        pytest.skip("real shards present but no npz+json pairs copyable")
    scan_or_update_manifest(samples_dir)
    ...
    tr = HexfieldTrainer(model=model, config=cfg, optimizer=opt)
    ...
    with warnings.catch_warnings():
        # Old-schema real shards (pre-policy_surprise) get skipped at load, leaving
        # an empty window; that is a data-availability condition, not a code fault,
        # so skip cleanly rather than fail (the p5 e2e gate owns the full path with
        # current-schema fixtures).
        warnings.simplefilter("ignore", RuntimeWarning)
        sel = tr.select_training_samples(ctx=ctx, components=comp, epoch=1)
    if sel["status"] != "completed" or comp.shared.sample_window.n == 0:
        pytest.skip(f"real shards unusable for a full pass (status={sel['status']})")
    ...
    diag_path = diag_dir / "hexfield.training.epoch_000001.json"
    assert diag_path.exists()
    assert not (diag_dir / "hexfield.training.epoch_000001.json.tmp").exists()
    disk = json.loads(diag_path.read_text(encoding="utf-8"))
