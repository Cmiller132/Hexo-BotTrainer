...
export PROBE_GAMES="${PROBE_GAMES:-256}"
export PROBE_PLIES="${PROBE_PLIES:-60}"
"$VIRTUAL_ENV/bin/python" "$ROOT/scripts/_tree_growth_probe.py" 2>&1 | grep -vE 'TracerWarning|UserWarning|if x.shape|frombuffer|default stream'
