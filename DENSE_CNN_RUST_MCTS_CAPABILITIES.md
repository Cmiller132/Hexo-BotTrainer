...
- `model1_mcts_policy_nucleus_widening` — each node may materialize only its
  top-prior moves covering `widening_policy_mass` cumulative prior (top-p),
  clamped to `[widening_min_children, widening_max_children]`. The cap is computed
  once from the prior at expansion (no visit-based growth). No hidden-prior mass.
...
- **Lazy-staged edges + nucleus widening:** a node holds its legal priors as
  `unexpanded_priors` and materializes an edge only when PUCT selects that
  candidate over the best existing edge, and only while
  `edges.len() < max_eligible_children` — the top-p (nucleus) cap computed once
  from the prior distribution at expansion (`nucleus_count`).
...
The earlier implementation carried Chaslot-style progressive widening (arbitrary
`growth_interval` / `growth_base` / dual initial-action knobs), a candidate-limited
half-precision evaluator mode with on-device top-k prior selection, hidden-prior
mass for omitted in-crop actions, lazy legal materialization, and tactical
win/block prior injection. All of that was removed; the evaluator now has a single
full-prior mode. Branching for the large action space is instead bounded by the
single-knob policy-nucleus (top-p) widening described above.
