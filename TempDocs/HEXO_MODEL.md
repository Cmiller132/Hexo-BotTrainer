...
## Model Package Composition

A model package brings whatever code its architecture needs. There is no
"Python-only by default, Rust as an exception" rule:

- **Python-only model packages.** Most architectures fit this shape. They
  define their network, choose an `ArchitectureSpec` (including which
  encoder/inference/training adapters they use), and rely on
  `hexo-utils` for the default 33×33×13 encoder, the neural MCTS engine,
  classical search, replay machinery, and shared adapter framework.
  Dense CNN, ResNet, attention, and graph architectures fit here today.
- **Model packages with their own Rust crate and PyO3 wheel.** Some
  architectures benefit from FFI-level work — high-volume state mutation,
  representation-coupled search, custom batched inference glue, or
  large internal data structures that would be slow if marshalled
  through Python. These packages bring a `hexgame-model-*` Rust crate
  and a matching PyO3 wheel alongside their Python code. V1 is the
  established example: `hexgame-model-v1` (Rust) ships V1 row-identity,
  pair-row contracts, and pair-native search; `hexorl-model-v1`
  (Python) ships players, training adapters, replay-construction
  glue, and diagnostics. Future model packages may follow the same
  pattern when justified.

Authority does not move with the model package. Even when a model package
ships its own Rust crate and runs its own search, legal move generation,
terminal detection, tactical analysis, and canonical game semantics still
come from `hexgame-engine` through the shared engine contract. A
model-owned Rust crate depends on `hexgame-engine` exactly the way
`hexgame-utils` does; it does not redefine rules.

Most model packages should still prefer the `hexo-utils` defaults — the
33×33×13 encoder, neural MCTS, classical search, and shared adapter
framework — when their representation and search needs fit those
abstractions. A model package should bring custom Rust or custom search
when the architecture's native representation or training targets cannot
be served by the defaults, not because going custom is the path of
prestige.

See `RUST_LAYOUT.md` for the Rust crate and PyO3 wheel layout.
