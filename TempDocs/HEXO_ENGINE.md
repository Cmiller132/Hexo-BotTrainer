...
- The default 33×33×13 board encoder. It is a model-side default and lives
  in `hexgame-utils`, not in `hexgame-engine`.
- Classical alpha-beta search or neural MCTS. Both live in `hexgame-utils`.
...
- V1 row-identity types, pair-row contracts, or pair-native search. V1 lives
  in `hexgame-model-v1`.
...
as state mutation, legal row generation, tactical analysis, and fixture
generation. It should expose compact batch APIs where useful, while leaving
search machinery, model tensor construction, and training batch preparation
outside the engine boundary. The Rust crate `hexgame-engine` is the concrete
home for this code; see `RUST_LAYOUT.md` for the crate split.
