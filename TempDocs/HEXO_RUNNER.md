...
- engine game records, whose schema is owned by `hexo-engine` (action history,
  terminal result, rules contract metadata, accepted transitions); the runner
  is the writer but does not own the schema;
- runner run metadata it owns outright (players, seeds, budgets, timings,
  outcomes, execution errors, provenance block);
- model-attached diagnostics whose schema is owned by `hexo-model-*` and
  transported by the runner as opaque metadata (search traces, candidate
  support, policy/value diagnostics);
- model-derived training records, owned and produced by the model adapter
  either during self-play or later during replay sampling — never by the
  runner.

The canonical ownership and writer roles for these layers are defined in
`up_next/STORAGE_IO.md`.
