...
Physical packaging exists at two levels: the Python packages above, and the
Rust crates / PyO3 wheels that back them. The Rust side mirrors the same
authority direction (`hexgame-engine` ← `hexgame-utils` ← `hexgame-model-*`)
and is specified in `Docs/new_project_structure/RUST_LAYOUT.md`. A model
package may bring its own Rust crate and PyO3 wheel when its architecture
benefits from FFI-level work; this is a normal option, not an exception.
...
- `hexo-runner` depends on engine contracts and the player interface only.
  Model and player packages are discovered through the registry at runtime
  rather than imported as static dependencies, so the runner stays unaware of
...
### V1 As The Worked Example

V1 is the established example of a model package that brings its own Rust
crate, its own PyO3 wheel, and its own search. This is a normal pattern, not
a special case the structure has to apologize for. The risk is the inverse:
if the boundaries become *so* loose that V1's Rust types or pair-search
internals leak back into `hexgame-engine` or `hexgame-utils`, the structure
is failing. The success criterion is that V1 lives entirely inside its own
package family while still consuming engine contracts cleanly.
...
- `Docs/new_project_structure/RUST_LAYOUT.md`
