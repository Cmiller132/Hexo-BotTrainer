...
The utility areas listed below span search bookkeeping, training-data plumbing,
serialization, telemetry, resource policy, and Rust-side test fixtures. These
do not share a common abstraction, and grouping them into a single flat module
would reproduce the dumping-ground failure mode this project explicitly warns
against.

`hexo-utils` should therefore be organized as a small set of internally
cohesive subpackages, even when distributed as one package:

- `hexo-utils.encoding` — the default 33×33×13 board encoder and its
  Python-side adapter glue;
- `hexo-utils.search` — neural MCTS engine, classical alpha-beta search,
  tree statistics, and traversal helpers;
- `hexo-utils.replay` — replay serialization, schema/version helpers, and
  training-adapter framework mechanics;
- `hexo-utils.runtime` — batching/queue helpers, resource profiles, and
  backpressure primitives;
- `hexo-utils.telemetry` — counters, timers, structured event helpers, and
  report fragments;
- `hexo-utils.rust_harness` — Rust-side mutator and test-harness helpers.

The Rust home for `encoding` and `search` is the `hexgame-utils` crate; see
`RUST_LAYOUT.md`.

A new utility belongs to `hexo-utils` only when it fits one of these
subpackages. Adding a new top-level subpackage should require the same scrutiny
as adding a new project family. If a utility does not fit, it belongs in a
model package or stays inside the consumer that needed it.

## Shared Utility Areas

### Default Board Encoder

`hexo-utils` ships the canonical 33×33×13 board encoder used by the dense /
ResNet / attention / graph model family today. The encoder is offered as a
default, not as a contract every model must obey: it is the easy path for
models whose representation fits a 33×33 crop with 13 feature channels.

Pluggability between encoders is handled at the existing Python adapter
layer (`ArchitectureSpec.input_contract_id`, `inference_adapter_id`,
`training_adapter_id`). Models that fit the default use the `crop:v1`
adapter. Models that need different inputs (graph features, sparse
representations, custom channel packs) implement their own adapters; those
adapters may call into the Rust default for shared steps (e.g., tactical
masks) or bypass it entirely. No new Rust trait is required to support
encoder pluggability — the adapter pattern already does this work.

The encoder is a `hexo-utils` deliverable rather than a model-specific
artifact because the 33×33 crop is shared infrastructure for an entire
family of architectures, not just one. The Rust home is the
`hexgame-utils` crate; see `RUST_LAYOUT.md`.

### Neural MCTS Engine

`hexo-utils` ships the neural MCTS engine used by the shared model family.
This is the actual search engine — node arena, PUCT selection, virtual
loss, batch leaf selection, expansion, backup — not just bookkeeping
helpers. Dense CNN, ResNet, attention, and graph-based architectures all
use the same engine.

The MCTS engine calls the default encoder directly inside the leaf-batch
hot path. This is intentional: the shared model family consumes the
33×33×13 tensor as input fan-in, and graph models compute supplementary
representations on top of the same Rust state without changing the MCTS
contract. Models that need fundamentally different MCTS leaf encoding (V1
pair search) bring their own search and do not reuse this engine.

Search consumers supply legal action sources, evaluator calls, value
interpretation, policy masking, and domain-specific stopping behavior
through explicit inputs and Python-side adapters. The MCTS engine does
not encode Hex authority, hard-coded tactical policy, or model-specific
priors.

### Classical Search

`hexo-utils` also ships the classical alpha-beta search used by scripted
opponents, evaluation baselines, and smoke tests. Like MCTS, it is shared
infrastructure rather than a model-specific algorithm.
